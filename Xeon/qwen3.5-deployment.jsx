export const Qwen35Deployment = () => {
  ...
  const XEON_SUPPORTED_MODELS = new Set(['397b', '122b', '35b', '27b', '9b', '4b', '2b', '0.8b']);
  ...
  const options = {
    ...
    hardware: {
      ...
      getDynamicItems: (values) => {
        ...
        const xeonDisabled = isNvfp4 || !XEON_SUPPORTED_MODELS.has(values.model);
        return [
          ...
          { id: 'xeon',   label: 'XEON',   default: false,     disabled: xeonDisabled,
            disabledReason: !XEON_SUPPORTED_MODELS.has(values.model) ? 'XEON support is not available for this model' : (isNvfp4 ? 'FP4 is only available on Blackwell GPUs' : '') }
        ];
      }
    },
    quantization: {
      ...
      getDynamicItems: (values) => {
        ...
        const hasFp4 = values.model === '397b' && !isXeon;
        return [
          { id: 'bf16', label: 'BF16', default: !hasFp8 || isXeon },
          { id: 'fp8',  label: 'FP8',  default: hasFp8 && !isXeon,  disabled: !hasFp8 || isXeon,
            disabledReason: isXeon ? 'FP8 is not supported on XEON for this page' : 'No FP8 variant available for this model' },
          { id: 'fp4',  label: 'FP4',  default: false,   disabled: !hasFp4,
            disabledReason: isXeon ? 'FP4 is only available on Blackwell GPUs' : 'FP4 is only available for Qwen3.5-397B-A17B' }
        ];
      }
    },
    reasoning: {
      ...
      getDynamicItems: (values) => {
        ...
        return [
          ...
          { id: 'enabled',  label: 'Enabled',  default: !isXeon, disabled: isXeon,
            disabledReason: 'Speculative decoding is not supported on XEON for this page' }
        ];
      }
    },
    mambaCache: {
  ...
  const generateCommand = () => {
    const { model, hardware, quantization, speculative, mambaCache, reasoning, toolcall } = values;

    const modelPath = quantization === 'fp4'
      ? 'nvidia/Qwen3.5-397B-A17B-NVFP4'
      : `Qwen/Qwen3.5-${MODEL_SUFFIX[model]}${quantization === 'fp8' ? '-FP8' : ''}`;

    if (hardware === 'xeon') {
      const xeonTp = (model === '397b' || model === '122b') ? 6 : 3;
      let cpuCmd = `sglang serve --model-path ${modelPath}`;
      cpuCmd += ` \
  --device cpu`;
      cpuCmd += ` \
