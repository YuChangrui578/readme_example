export const Qwen35Deployment = () => {
  ...
  const XEON_MODELS = new Set(['397b', '122b', '35b', '27b', '9b', '4b', '2b', '0.8b']);
      ...
      getDynamicItems: (values) => {
        ...
        return [
          ...
          { id: 'fp4',  label: 'FP4',  default: false,   disabled: !hasFp4 || isXeon,
            disabledReason: isXeon ? 'FP4 is not available on Intel CPU Xeon' : 'FP4 is only available for Qwen3.5-397B-A17B' }
        ];
      }
    },
    reasoning: {
    ...
    speculative: {
      ...
      getDynamicItems: (values) => {
        ...
        return [
          ...
          { id: 'enabled',  label: 'Enabled',  default: false, disabled: isXeon }
        ];
      }
    },
    mambaCache: {
  ...
  const generateCommand = () => {
    ...
    if (hardware === 'xeon') {
      let cpuCmd = `sglang serve --model-path ${modelName} \
  --device cpu \
  --tp ${tpValue}`;
